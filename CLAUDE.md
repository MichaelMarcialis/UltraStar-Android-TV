...
Note `MainActivity` currently launches `diagnostics/SyncCalibrationScreen.kt`, a test harness rather than real UI; `diagnostics/IsoCaptureScreen.kt` is the other one, for mic capture and pitch alone. Both open their mics through `mic/UsbMicSession.kt`, which owns the asynchronous USB permission dance — gameplay will want it too.
