...
- **Gameplay UI** (`game/`) — the game itself, verified on hardware in both layouts. `GameSession` owns the player, the mics and a `PlayerScorer` per singer; `NoteTrack` draws; `GameTheme` holds every colour and dimension in one file so the look can be tuned without reading drawing code.
  ...
  - **The vertical span is fixed for the whole song; only the centre moves** (`PitchRange`). Two wrong answers preceded this. Scaling to the whole song squashed every phrase into a third of the height — "Free" spans 22 semitones (pitch 3–25, both used repeatedly, not an outlier). Resizing to fit each passage fixed that and introduced a worse problem: the notes visibly stretched and squashed as the melody moved, which the user picked out immediately as distracting. So the span is decided once per song and the view only *pans*, with a deadband that keeps it perfectly still until notes press against an edge. **A constant scale means a semitone is always the same distance on screen**, which is what lets the eye learn the shape of a phrase.
  - The span is sized from the **ninetieth percentile of the per-line ranges**, not the song's extremes: one screamed high note must not shrink every verse for the whole song.
  ...
  - Arrow smoothing is deliberately **fast (~50 ms)**: this is an instrument, not a decoration, and a singer correcting their pitch must not see the arrow agree late. Silence hides the arrow rather than freezing it — "not singing" must not look like "singing the same note" — but the position is *remembered* through short gaps, so a breath mid-phrase does not cause a snap.
  ...
  - **There is a third latency, and it runs the other way** (`SyncCalibration.displayLeadSeconds`). Output and capture latency both describe *sound* that has already happened, so both are subtracted. The delay between drawing a frame and the TV showing it describes *light that has not been seen yet*, so it is **added**. Missing it showed up exactly as reported from the sofa: the lyric is sung a moment before it reaches the line. **It is still `0.0` — nothing has measured it.** Up/down on the D-pad dials it in live and the value is on screen; raise it until the lyric meets the line as it is sung, then bake that number in the way the 127 ms was. Guessing a default would only make the eventual measurement harder to trust.
