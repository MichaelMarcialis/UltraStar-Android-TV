...
**Not started:** TV browse UI, two-player gameplay UI, settings.

**Next up:** the gameplay UI — the last big piece, and the first time all of this is a game rather than a set of harnesses. Everything it needs now exists: a library to pick from, a player and clock, a scorer, and a measured sync offset (remember lyrics render against `heardSongTimeFor`, not the raw player position).
...
Note `MainActivity` currently launches `diagnostics/SongLibraryScreen.kt`, a test harness rather than real UI; the others are `diagnostics/SyncCalibrationScreen.kt` (latency, and the end-to-end proof) and `diagnostics/IsoCaptureScreen.kt` (mic capture and pitch alone). Swap the one line in `MainActivity` to switch. Both open their mics through `mic/UsbMicSession.kt`, which owns the asynchronous USB permission dance — gameplay will want it too.
